...
const walkDir = dir => fs.promises.readdir(dir).then(list => Promise.all(list.map(async file => {
  ...
  if ((await fs.promises.stat(cur)).isDirectory()) await walkDir(cur)
    ...
    files.push([cur, name.startsWith('electron') ? 'UnityDetectorX' + name.replace(/^electron/, '') : name])
  }
})))
...
walkDir(electronRoot)
  .then(() => Promise.all(files.map(it => fs.promises.readFile(typeof it === 'string' ? it : it[0]).then(data => zip.file('UnityDetectorX/' + (typeof it === 'string' ? it : it[1]), data)))))
